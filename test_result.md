...
agent_communication:
    ...
    -agent: "main"
    -message: "PRODUCTION-READY 7-IN-1 BATCH — Re-validation requested. (1) Legal Compliance Gate déjà livré tour précédent : PaywallScreen.tsx avec checkbox + hyperliens CGV/Privacy + bouton disabled until checked. Pages /cgv et /privacy créées. (2) Project Validation Button : carte verte testID 'validate-for-fabrication-button' sous Photos Site, transitionne a_verifier/technique_a_valider → en_fabrication via PATCH /chantiers/{id} {status:'en_fabrication'}. (3) Team Size Logic : frontend computed teamSize = users.length ; solo (≤1 user) ou artisan_mode → admin peut valider ; sinon seul Technicien peut valider, Admin/Commercial voient card 'EN ATTENTE DE VALIDATION PAR LE TECHNICIEN' (l'admin NE peut PAS bypasser). (4) Manufacturing Locks + Tech Override : en status='en_fabrication', frontend masque MODIFIER/SUPPRIMER/AJOUTER OUVERTURE pour tous sauf Tech avec override actif. Backend mesures POST/PATCH/DELETE n'a PAS de restriction supplémentaire sur en_fabrication (pour permettre override Tech). (5) Auto Team Notifications : ajout `send_assignment_email()` dans /app/backend/email_service.py ; appelé dans routes/chantiers.py create_chantier quand payload.assigned_to != user.id (anti-double + try/except non-bloquant). (6) Feedback Recipient : routes/feedbacks.py create_feedback déclenche send_feedback_email() vers SUPPORT_EMAIL env (default support@mesurechassis.fr) avec contexte sender_email, sender_name, company_name, page_context, user_comment ; try/except non-bloquant. (7) CSV MIME fix : changement frontend uniquement (Sharing.shareAsync avec mimeType='text/csv' + UTI 'public.comma-separated-values-text'). Tests existants : 167/167 PASS. Identifiants : admin@mesurechassis.fr/admin123 (master admin), commercial@mesurechassis.fr/commercial123, tech@mesurechassis.fr/tech123. Le pre-check : artisan_mode est ACTIF (true) par défaut sur la société 'default' — bypass total des RBAC. Pour tester la matrice stricte, désactiver via PATCH /api/company/profile {artisan_mode:false} avec token admin puis RESTORE artisan_mode=true à la fin. À tester précisément : (T1) GET /api/feedbacks après POST → la réponse doit retourner correctement et le log backend doit contenir un email 'Subject: [Feedback]' avec page_context et user_comment. (T2) POST /api/chantiers avec assigned_to pointant vers un autre user que le créateur → vérifier dans backend logs 'Subject: Nouveau chantier attribué :' avec assigné_name + chantier_name + address. (T3) Self-assignment (assigned_to == user.id) ne doit PAS envoyer d'email (anti-double). (T4) PATCH /chantiers/{id} {status:'en_fabrication'} : admin peut (le backend n'a pas de restriction backend, le frontend gate via teamSize). Vérifier 200. (T5) Régression complète : 167/167 pytest existants doivent toujours passer (cd /app/backend && python -m pytest tests/ --no-cov). (T6) Cleanup obligatoire : PATCH artisan_mode=true à la fin pour ne pas bloquer l'utilisateur."
