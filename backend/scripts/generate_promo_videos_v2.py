...
LOGIN_SCREEN_DESCRIPTION = (
    "The smartphone screen displays the MesureChâssis app LOGIN SCREEN, "
    "vertically oriented, with a PURE BLACK background. From top to "
    "bottom: (1) Centered at the top, a large ORANGE rounded square "
    "logo icon containing two black arrows pointing in opposite "
    "diagonal directions (one arrow toward upper-left, one arrow toward "
    "lower-right, like an 'expand' or 'resize' icon). (2) Just below the "
    "logo, the bold white uppercase title 'MESURECHÂSSIS'. (3) Below "
    "that, light gray subtitle text 'Mesures terrain · Menuiseries pro'. "
    "(4) Below that, a horizontal toggle pill with two segments: the "
    "left segment is solid ORANGE (#FF5A00) with black bold text "
    "'CONNEXION' (active state), the right segment is dark gray with "
    "white text 'INSCRIPTION' (inactive). (5) Below, label 'EMAIL' in "
    "small light-gray uppercase, then a dark rounded input field with "
    "placeholder text 'prenom.nom@entreprise.fr' in gray. (6) Below, "
    "label 'MOT DE PASSE' and another dark input field showing 7 small "
    "dots (•••••••) with a white eye-icon on the right. (7) Below, a "
    "LARGE prominent ORANGE rounded button with bold black text 'SE "
    "CONNECTER'. (8) At the bottom, orange small text 'Mot de passe "
    "oublié ?'. Modern dark theme professional B2B SaaS mobile app "
    "design, clean spacing, premium feel. "
)
...
VIDEO_1_CLIPS = [
    {
        ...
        "prompt": base_prompt(
            ...
            "windows. He picks up the notebook with one hand AND picks up "
            "his smartphone with the other hand — both items are clearly "
            "visible in his hands as he walks toward the exit door with a "
            "confident, relaxed smile. CRITICAL ELEMENT: a single blue "
            "ballpoint pen sits on the corner of the desk, BUT the "
            "carpenter NEVER LOOKS at the pen — his gaze is fixed forward "
            "toward the door, he walks past the desk and leaves the pen "
            "completely unnoticed. The camera briefly shows the abandoned "
            "pen on the desk after he has already walked past it. He exits "
            "the office with the notebook and smartphone in hand, hood "
            "down. Cinematic shallow depth of field."
        ),
    },
    {
    ...
    },
    {
        ...
        "prompt": base_prompt(
            ...
            "still wearing his HV jacket (hood down). He looks slightly "
            "frustrated and tired, shoulders dropped, holding a crumpled "
            "paper notebook in one hand. A second character — a friendly "
            "colleague (a different European man in his 30s, wearing a "
            "casual blue carpenter polo, clean and smiling face) — "
            "approaches him from the side and warmly shows him a "
            "smartphone screen. The smartphone screen displays a clean "
            "mobile app interface with orange accent color (#FF5A00) "
            "showing a list of window measurements. CRITICAL DETAIL: The "
            "main carpenter ATTENTIVELY LOOKS AT and FOCUSES HIS GAZE "
            "DIRECTLY ON the smartphone screen the colleague is showing "
            "him — his eyes are clearly directed at the phone screen, NOT "
            "at the colleague's face. His eyes widen with surprise and "
            "curiosity as he studies the app interface, a small smile "
            "starting to form on his closed lips. Warm interior workshop "
            "lighting."
        ),
    },
]
...
VIDEO_2_CLIPS = [
    ...
    },
    {
        ...
        "prompt": base_prompt(
            ...
            "calm. He gently places his smartphone face-up in the "
            "FOREGROUND on his clean organized desk, screen facing the "
            "camera. CRITICAL: the smartphone screen is CLEARLY VISIBLE "
            "and TURNED ON throughout the entire shot. The phone occupies "
            f"the front-left of the frame, bright and detailed. "
            f"{LOGIN_SCREEN_DESCRIPTION}The camera lingers on the lit "
            "phone screen so the viewer clearly reads the MesureChâssis "
            "brand and the orange 'SE CONNECTER' button. In the "
            "background, the carpenter stretches his arms above his head "
            "in a satisfied 'mission accomplished' gesture, picks up a "
            "coffee mug, sits down in his office chair. Hero final brand "
            "reveal shot, golden-hour uplifting serene mood. The phone "
            "screen must NEVER go dark or black — it stays brightly lit "
            "showing the MesureChâssis login screen for the entire clip."
        ),
    },
]
